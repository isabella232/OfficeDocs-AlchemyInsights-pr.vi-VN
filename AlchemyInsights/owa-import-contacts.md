---
title: '8000019: cách thêm hoặc nhập liên hệ'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 7eb53224bb3370fc434e520d69910d0c9c23f501
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: vi-VN
ms.lasthandoff: 09/14/2020
ms.locfileid: "47694081"
---
# <a name="import-contacts"></a>Nhập liên hệ

Trước khi nhập liên hệ vào Outlook trên web, bạn sẽ cần xuất chúng từ nhà cung cấp email khác của bạn dưới dạng tệp. CSV. Sau đó, hãy làm theo các hướng dẫn dưới đây.

1. Ở cuối trang, chọn <img src='data:image/png;base64,' />
 **Mọi người**.
2. Trên thanh công cụ, chọn **quản lý**  >  **nhập liên hệ**.
3. Chọn **duyệt**, chọn tệp CSV của bạn, rồi chọn **mở**.
4. Chọn **nhập**.

Để tìm hiểu thêm, hãy xem [nhập liên hệ vào Outlook](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web).

