---
title: Thử nghiệm Alchemy inline Images
ms.author: pebaum
author: pebaum
ms.date: 11/16/2017
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 28d8036f-9df2-4dff-835b-9424bfe988a2
ms.custom:
- "273"
- "1500006"
ms.openlocfilehash: 8a9d98e2af3ef0cc71b820b4e74aa2a2a522346b
ms.sourcegitcommit: 0b06093dabd685f76cc39b1d7c0f8b03883b6e79
ms.translationtype: MT
ms.contentlocale: vi-VN
ms.lasthandoff: 10/25/2019
ms.locfileid: "36743151"
---
# <a name="access-the-domains-page"></a>Truy cập trang miền

Để truy cập trang miền, chỉ cần nhấp vào biểu tượng dưới đây:

  
<img src='data:image/png;base64,'/>

Sau đó, bạn có thể thêm, sửa đổi hoặc xóa tên miền của mình.
Để biết thêm thông tin, hãy xem [câu hỏi thường gặp về miền.](https://docs.microsoft.com/office365/admin/setup/domains-faq)
  